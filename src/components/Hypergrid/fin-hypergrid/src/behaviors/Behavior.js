...
var Behavior = Base.extend('Behavior', {
    ...
    setPSP: function (model) {
      this.grid.properties.fixedColumnCount = model._axisV.isTree ? 1 : (model._axisV.currentWidth() + 1) ;

      const new_schema = [];
      const f = this.grid.properties.fixedColumnCount ;
      ...
      for (var i = 0; i < f; ++i) {
          ...
          new_schema.push({ name, header, type, index: i });
      }

      if (model._axisH) {
        model._axisH._axis.forEach(function (columnPath, columnIndex) {
            const header = ' ',
                name = (columnIndex + i).toString(),
                type = "float";
      ...
      this.createColumns  = this.createColumnsOLAP;
      this.createColumns();
    },
    ...
    setHeadersOLAP: function (headers) {
      const axisH = this.grid.behavior.dataModel._axisH ;
      if (!axisH)
        return ;
