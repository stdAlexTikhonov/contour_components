...
var Renderer = Base.extend('Renderer', {
    ...
    paint: function(gc) {
        if (this.grid.canvas) {
//            if (this.grid.chartDiv)
//                this.grid.chartDiv.style.display = 'none' ;
            this.renderGrid(gc);
            this.grid.gridRenderedNotification();
            if (this.grid.chartDiv && this.grid.chartOptionSet === false) {
                this.grid.chart.setOption(this.grid.chartOption, true) ;
                this.grid.chartOptionSet = true ;
            }
//            if (this.grid.chartDiv)
//                this.grid.chartDiv.style.display = '' ;
        }
    },
    ...
    getFinalVisibleColumnBoundary: function() {
        ...
        var colWall = this.visibleColumns[Math.max(0, this.visibleColumns.length - chop)].right;
    ...
    renderLastSelection: function(gc) {
        ...
        var config = {
            bounds: {
                x: vcOrigin.left,
                y: vrOrigin.top,
                width: vcCorner.right   - vcOrigin.left - gridProps.selectionRegionOutlineWidth / 2,
                height: vrCorner.bottom - vrOrigin.top  - gridProps.selectionRegionOutlineWidth / 2
            },
            ...
            selectionRegionOutlineWidth: gridProps.selectionRegionOutlineWidth
        };
    ...
    renderOverride: function(gc, override) {
        ...
        var imgData = gc.getImageData(Math.round(startX * hdpiRatio), Math.round(startY * hdpiRatio), Math.round(width * hdpiRatio), Math.round(height * hdpiRatio));
        ...
        if (C && R) {
            ...
            if (
                ...
                var gridLinesVWidth = gridProps.gridLinesVWidth,
                    ...
                    firstDataRow = visibleRows[headerRowCount - isShowFactHeading], // any data rows?
            ...
            if (
                ...
                var gridLinesHWidth = gridProps.gridLinesHWidth,
                    headerRowCount = this.grid.getHeaderRowCount(),
                    left = gridProps.gridLinesRowHeader ? 0 : visibleColumns[Math.min(visibleColumns.length - 1, gridProps.fixedColumnCount)].left,
                    right = gridProps.gridLinesUserDataArea ? viewWidth : rowHeader.right;

                gc.fillStyle = gridLinesHColor;

                visibleRows.forEach(function(vr, r) {
                    if (r < R1 && r > headerRowCount - isShowFactHeading) { // don't draw rule below last row
                        var y = vr.bottom;
                        if (borderBox) {
                            y -= gridLinesHWidth;
                        }
            ...
            var gap;
            var gapRows = visibleRows.gap ;
            var gapColumns = visibleColumns.gap ;
            
            var y0 = gapRows ? gapRows.bottom : 0 ;
            var x0 = gapColumns ? gapColumns.left : 0 ;
            
            if (gridProps.fixedLinesHWidth !== undefined) {
                if ((gap = gapRows)) {
                    gc.cache.fillStyle = gridProps.fixedLinesHColor || gridLinesHColor;

                    gc.fillRect(x0, visibleRows[0].bottom, viewWidth - x0, gap.bottom - gap.top);
                    gc.fillRect(0, y0, viewWidth, gap.bottom - gap.top);
                    gc.fillRect(0, viewHeight, viewWidth, gap.bottom - gap.top);
                }
            }
            ...
            if (gridProps.fixedLinesVWidth !== undefined) {
                if ((gap = gapColumns)) {
                    ...
                    gc.fillRect(0, y0, gap.right - gap.left, viewHeight - y0);
                    gc.fillRect(x0, visibleRows[0].bottom, gap.right - gap.left, viewHeight - visibleRows[0].bottom);
                    gc.fillRect(viewWidth, visibleRows[0].bottom, gap.right - gap.left, viewHeight - visibleRows[0].bottom);
                }
            }
        }
    },
    ...
    _paintCell: function(gc, cellEvent, prefillColor) {
        ...
        cellEvent.selectRect = config.selectRect;
...
function computeCellsBounds() {
    ...
    if (this.grid.chartDiv) {
        const x = this.visibleColumns.gap?.right,
              y = this.visibleRows.gap?.bottom,
              w = Math.min(this.grid.getBounds().corner.x, this.visibleColumns[this.visibleColumns.length - 1]?.right) - 1,
              h = Math.min(this.grid.getBounds().corner.y, this.visibleRows[this.visibleRows.length - 1]?.bottom) - 1;
        this.grid.chartDiv.style.top    = y + 'px';
        this.grid.chartDiv.style.left   = x + 'px';
        this.grid.chartDiv.style.width  = w - x + 'px';
        this.grid.chartDiv.style.height = h - y + 'px';
        this.grid.chart.resize({ opts: { width: w - x + 'px', height: h - y + 'px' } });
    }
